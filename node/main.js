...
function Block1() {
  ...
  for (let it = 0; it < 100000; it++) {
    ...
    // 1     = *... .... .... *... .... *... ..** ....  0x80080830
    // Q[3]  = .... .... .*** .*** **** .*** *... ....  0x0077f780
    Q[4] = ((((rng() & 0x7f00000f) >>> 0) + 0x80080830 + ((Q[3] & 0x0077f780) >>> 0))>>>0) >>> 0;
    ...
    // 0     = .*** .**. *.** **** **** **** **.* *.*.  0x76bfffda
    // 1     = *... *... .*.. .... .... .... ..*. .*.*  0x88400025
    Q[5] = ((rng() & 0x01000000) + 0x88400025) >>> 0;
    ...
    // 1     = .... ..*. .*** **** *.** **.. .*.. ...*  0x027fbc41
    // Q[5]  = .... ...* .... .... .... .... .... ....  0x01000000
    Q[6] = (0x027fbc41 + (Q[5] & 0x01000000)) >>> 0;
    ...
    // 0     = **** ***. .... ***. *.*. *.*. *.** ****  0xfe0eaabf
    // 1     = .... ...* *..* ...* .... .*.* .*.. ....  0x01910540
    Q[8] = ((rng() & 0x00605000) + 0x01910540) >>> 0;
    ...
    // 1     = **** *.** ...* .... ..*. **** ..** **.*  0xfb102f3d
    // Q[8]  = .... .... .... .... ...* .... .... ....  0x00001000
    Q[9] = ((rng() & 0x00e04000) + 0xfb102f3d + (Q[8] & 0x00001000)) >>> 0;

    // Q[10] = 0111 .... 0001 1111 1v01 ...0 01.. ..00
    // RNG   = .... **** .... .... .*.. ***. ..** **..  0x0f004e3c
    // 0     = *... .... ***. .... ..*. ...* *... ..**  0x80e02183
    // 1     = .*** .... ...* **** *..* .... .*.. ....  0x701f9040
    Q[10] = ((rng() & 0x0f004e3c) + 0x701f9040) >>> 0;
    ...
    // 1     = ..*. .... ***. ...* *... .... **.. ..*.  0x20e180c2
    // Q[10] = .... .... .... .... .*.. .... .... ....  0x00004000
    Q[11] = ((rng() & 0x0a100a3c) + 0x20e180c2 + (Q[10] & 0x00004000)) >>> 0;
    ...
    // 1     = .... .... .... *... ...* ...* .... ....  0x00081100
    // Q[11] = .... ..** .... .... .... .... .... ....  0x03000000
    Q[12] = ((rng() & 0x1cf00e7f) + 0x00081100 + (Q[11] & 0x03000000)) >>> 0;

    // Q[13] = 01.. ..01 .... 1111 111. ...0 0... 1...
    // RNG   = ..** **.. **** .... ...* ***. .*** .***  0x3cf01e77
    // 0     = *... ..*. .... .... .... ...* *... ....  0x82000180
    // 1     = .*.. ...* .... **** ***. .... .... *...  0x410fe008
    Q[13] = ((rng() & 0x3cf01e77) + 0x410fe008) >>> 0;

    // Q[14] = 000. ..00 .... 1011 111. ...1 1... 1...
    // RNG   = ...* **.. **** .... ...* ***. .*** .***  0x1cf01e77
    // 0     = ***. ..** .... .*.. .... .... .... ....  0xe3040000
    // 1     = .... .... .... *.** ***. ...* *... *...  0x000be188
    Q[14] = ((rng() & 0x1cf01e77) + 0x000be188) >>> 0;

    // Q[15] = v110 0001 ..V. .... 10.. .... .000 0000
    // RNG   = *... .... **** **** ..** **** *... ....  0x80ff3f80
    // 0     = ...* ***. .... .... .*.. .... .*** ****  0x1e00407f
    // 1     = .**. ...* .... .... *... .... .... ....  0x61008000
    Q[15] = ((rng() & 0x80ff3f80) + 0x61008000) >>> 0;
    ...
    // 1     = .*.. .... .... .... .... .... .... ....  0x40000000
    // Q[16] = *... .... .... .... *... .... .... *...  0x80008008
    Q[17] = ((rng() & 0x3ffd7ff7) + 0x40000000 + (Q[16] & 0x80008008)) >>> 0;
    ...
    x[1] =
      ...
      0;
    x[4] = (RR(Q[5] - Q[4], 7) - F(Q[4], Q[3], Q[2]) - Q[1] - 0xf57c0faf) >>> 0;
    x[5] =
      (RR(Q[6] - Q[5], 12) - F(Q[5], Q[4], Q[3]) - Q[2] - 0x4787c62a) >>> 0;
    x[6] =
      (RR(Q[7] - Q[6], 17) - F(Q[6], Q[5], Q[4]) - Q[3] - 0xa8304613) >>> 0;
    x[10] =
      (RR(Q[11] - Q[10], 17) - F(Q[10], Q[9], Q[8]) - Q[7] - 0xffff5bb1) >>> 0;
    x[11] =
      (RR(Q[12] - Q[11], 22) - F(Q[11], Q[10], Q[9]) - Q[8] - 0x895cd7be) >>> 0;
    x[15] =
      (RR(Q[16] - Q[15], 22) - F(Q[15], Q[14], Q[13]) - Q[12] - 0x49b40821) >>>
      0;

    // Q[2] = .... .... .... .... .... .... .... ....
    Q[2] = (Q[1] + RL(F(Q[1], QM0, QM1) + QM2 + x[1] + 0xe8c7b756, 12) >>> 0) >>> 0;

    // Q[18] ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Q[18] =
      (Q[17] + RL(G(Q[17], Q[16], Q[15]) + Q[14] + x[6] + 0xc040b340, 9) >>> 0) >>> 0;
...
if (process.argv.length > 2) {
  X = parseInt(process.argv[2], 16);
}
console.log('seed', X >>> 0);
if(Block1() == 0) {
  if(Block2() == 0){
    console.log('Colliding hash', calculateHash());
    process.exit(0)
  } else {
    console.warn("NO BLOCK2 FOUND");
  }
} else {
  console.warn("NO BLOCK1 FOUND");
}
process.exit(1)
